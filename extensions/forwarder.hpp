...
public: // allow enabling ndnSIM content store (will be removed in the future)
  ...
  void onARTExpire(const shared_ptr<pit::Entry>& pitEntry, time::milliseconds dataFreshnessPeriod = time::milliseconds(-1));

  void onDoneAwaitingResponse(const shared_ptr<pit::Entry>& pitEntry);

  void
  setAwaitingResponseTimer(const shared_ptr<pit::Entry>& pitEntry, bool isSatisfied,
                    time::milliseconds dataFreshnessPeriod = time::milliseconds(-1));

  void
  clearAwaitingResponseTimer(pit::Entry& pitEntry);

  void
  cancelAwaitingResponseTimer(pit::Entry& pitEntry);
