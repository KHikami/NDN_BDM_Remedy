...
bool
canForwardExclude(const pit::Entry& pitEntry, const Face& currFace)
{
  vector<FaceId> prevResponded = pitEntry.getRespondedFaces();
  for(unsigned int i = 0; i < prevResponded.size(); i++)
  {
     if(currFace.getId() == prevResponded.at(i))
     {
        return true;
     }
  }
