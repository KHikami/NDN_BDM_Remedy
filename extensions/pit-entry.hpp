...
public:
  ...
  //unfortunately CS can only do deletion on the name!
  //would like to store the face that gave the infected data... but may not be helpful...
  const Data&
  getRespondedData()
  {
    return *m_respondedData;
  }

  bool
  hasRespondedData()
  {
    return m_respondedData != nullptr;
  }


  void
  setRespondedData(shared_ptr<const Data> data)
  {
     m_respondedData = data;
  }
...
public:
  ...
  //timer for the forwarding pipeline to delete the pit entry if there wasn't an exclude flag returned for the current
  //bad data
  scheduler::EventId m_awaitingResponseTimer;

private:
  shared_ptr<const Data> m_respondedData;
  bool m_awaitingRemedy;
